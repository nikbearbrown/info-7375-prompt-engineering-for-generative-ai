# Research notes — Tools, permissions, and boundaries
...
INFO 7375 course contributors, Tools, permissions, and boundaries, code/main.py (course adaptation 2026), [implementation](../lessons/05-tools-and-permissions/code/main.py). Read in full during this pass. Companion [lesson](../lessons/05-tools-and-permissions/docs/en.md) and [tests](../lessons/05-tools-and-permissions/code/tests/test_main.py) remain scheduled for full chapter-specific reading. The course code is primary evidence of its own behavior, not of Claude internals or security guarantees.

## Claim ledger

1. **C05-1:** authorize resolves root and target and rejects a resolved target outside the root. Source: the local implementation above; inspection-supported, awaiting review.

2. **C05-2:** A write requires approved is True; the function returns a Path. Source: the local implementation above; inspection-supported, awaiting review.

3. **C05-3:** It neither performs the operation nor authenticates the approver; preflight checking is not a complete filesystem sandbox. Source: the local implementation above; inspection-supported, awaiting review.
...
See [reproducible script](worked_examples.py), [recorded output](worked-examples.json) (key `chapters.05`), [primary source register](sources.md), and [scope and review packet](review-packet.md). Optional companion notes and new empirical claims require passage-specific support; unsupported material is excluded, not silently marked verified.
